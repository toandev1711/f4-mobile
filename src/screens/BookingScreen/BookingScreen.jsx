...
const PASSENGER_VEHICLES = [
  { id: 1, name: "Xe máy", price: 7000, description: "Tối đa 1 khách" },
  { id: 2, name: "Xe 4 chỗ", price: 12000, description: "Tối đa 4 khách" },
  { id: 3, name: "Xe 7 chỗ", price: 16000, description: "Tối đa 7 khách" },
];
...
const CARGO_VEHICLES = [
  ...
  },
  {
    ...
    name: "Xe tải",
    price: 50000,
    description: "Phù hợp với hàng hóa lớn, cồng kềnh",
  },
];
...
const BookingScreen = () => {
  ...
  const { pickUp, dropOff } = useLocation();
  ...
  const { bookingId, setBookingId, setBooking } = useAuth();
  const handleBook = async () => {
    ...
    try {
      ...
      if (response.code !== 1000) {
        alert("Hiện tại không tìm thấy tài xế, vui lòng thử lại sau.");
      }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <ServiceDetails
        pickup={pickUp.displayName}
        destination={dropOff.displayName}
      ...
      {transportType === "cargo" && (
        <CargoInfo
          ...
          onUpdate={(field, value) =>
            setCargoInfo((prev) => ({ ...prev, [field]: value }))
          }
        />
      )}
